...
## read `r names(DD.metaSheetsHdr.vec)[3]`


```{r m1a-readTargFragCancer, eval=T, cache=T, cache.vars=c('DD.targFragCancer.frm'),warning=F}
DD.targFragCancer.frm <- as.data.frame(
 readxl::read_xlsx(DD_META_FILE, sheet=DD.metaSheets.vec[3], skip=1))

names(DD.targFragCancer.frm) <- 
sub("Patient Type", "patientType",
sub("Sample Type", "sampleType",
sub("Age at Diagnosis", "Age",
sub("TNM Staging", "TMM_Staging",                            
